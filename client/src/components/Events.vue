      ...
      <v-card class="cardAlert">
        <v-card-actions>
          ...
          <v-btn
            ...
            dark
            color="cyan"
            @click="submitApplicationDialog = false"
          >
    ...
    <div class="col-md-12">
      <v-card>
        ...
        <v-card-text style="padding-left: 30px; line-height: 1.6;">
          <h3>
            Female entrepreneurs are disadvantaged in traditional funding
            markets.We attempt to change this status quo by giving alternative
            access while empowering investors in general with data and education
            on gender lens investing across asset classes.
          </h3>
        </v-card-text>
        <v-data-iterator
          :items="items"
          ...
          <template v-slot:header>
            <v-toolbar dark color="grey darken-4" class="mt-1">
              <v-text-field
                ...
                label="Search for an Upcoming Event"
              >
              </v-text-field>
            ...
            <v-row>
              ...
              >
                <v-card class="my-5" max-width="380">
                  <v-img height="200" :src="item.images"> </v-img>
                  <v-card-title class="pb-2 text-justify"
                    >{{ item.name | truncate(27, "...") }}
                  </v-card-title>
                  <v-card-text>
                    ...
                    <div class="font-weight-medium">
                      Total funds required (MYR): {{ item.totalvolunteer }}
                    </div>
                    <div>
                      Closing date: {{ getLocaleDate(item.closing, true) }}
                    </div>
                    <div>
                      Location: {{ item.location | truncate(18, "...") }}
                    </div>
                  </v-card-text>
                  ...
                  <v-card-actions>
                    ...
                    <v-switch
                      ...
                      :label="isExpanded(item) ? 'Hide' : 'More..'"
                      class="expand"
                  ...
                  <v-expand-transition>
                    <div v-if="isExpanded(item)">
                      <v-card-text>
                        ...
                        <h4>({{ item.location }})</h4>
                        Contact Person/Num:
                        <p>{{ item.contactname }} ({{ item.contactnum }})</p>
                        <p v-if="item.datefrom != null">
                          Date(s): {{ getLocaleDate(item.datefrom, true) }} -
                          {{ getLocaleDate(item.dateto, true) }}
                        </p>
                        <v-spacer></v-spacer>
                        {{ item.description }}
                      </v-card-text>
