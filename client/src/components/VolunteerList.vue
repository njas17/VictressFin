  ...
  <v-container>
    <v-dialog v-model="deleteDialog" max-width="300px" title="Delete">
      ...
      <v-card>
        <v-card-text>
          <v-row class="mx-0">
            This investor/mentor record will be deleted. Are you sure?
          </v-row>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          ...
          <v-btn
            color="blue darken-1"
            text
            @click="handleDelete(selectedVolunteer)"
          >
            Yes
          </v-btn>
        </v-card-actions>
      ...
      >
        ...
        <template v-slot:[`item.action`]="{ item }">
          ...
          <v-icon
            ...
            small
            class="mr-2"
            @click="updateStatus(item.vid, 'declined')"
          >
            mdi-cancel
          </v-icon>
          <v-icon
            small
            color="blue darken-3"
            @click="openDeleteDialog(item.vid)"
          >
            mdi-delete
          </v-icon>
        </template>
