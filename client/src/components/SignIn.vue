  ...
  <v-card fill-height>
    <v-dialog v-model="nonMemberDialog" max-width="300px" title="Delete">
      ...
      <v-card>
        <v-card-text>
          <v-row class="mx-0">
            You profiles are not registed in our system. Please click on the
            Sign-Up tab to register as our member.
          </v-row>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
  ...
  methods: {
    userSignIn() {
      if (this.email === "" || this.password === "") {
        this.errorMesg =
          "All fields are required. Please provide the information.";
        return;
      }
    ...
    login() {
      fetch("/api/auth/users/" + this.email)
        ...
        .then((data) => {
          if (data.length < 1)
            throw "User profile not found with the entered credentials.";
