  ...
  <v-card>
    <ValidationObserver ref="observer">
      <v-form ref="form" lazy-validation style="padding: 30px;">
          ...
          <v-row align="center">
            <v-col cols="12" sm="6" md="6">
              <v-dialog
                ref="dialog"
                v-model="smodal"
                :return-value.sync="currentevent.datefrom"
                persistent
                width="290px"
              >
                ...
                <v-date-picker
                  ...
                  scrollable
                >
                  <v-btn text color="info" @click="smodal = false"
                    >Cancel</v-btn
                  >
                  <v-btn
                    text
                    color="info"
                    @click="$refs.dialog.save(currentevent.datefrom)"
                    >OK</v-btn
                  >
                </v-date-picker>
              </v-dialog>
            ...
            <v-col cols="12" sm="6" md="6">
              <v-dialog
                ref="dialog2"
                v-model="emodal"
                :return-value.sync="currentevent.dateto"
                persistent
                width="290px"
              >
                <template v-slot:activator="{ on, attrs }">
                ...
                >
                  <v-spacer></v-spacer>
                  <v-btn text color="info" @click="emodal = false"
                    >Cancel</v-btn
                  >
                  <v-btn
                    text
                    color="info"
                    @click="$refs.dialog2.save(currentevent.dateto)"
                    >OK</v-btn
                  >
                </v-date-picker>
              </v-dialog>
          ...
          <v-row>
            <v-col cols="12" sm="6" md="6">
              <v-dialog
                ref="dialog3"
                v-model="cmodal"
                :return-value.sync="currentevent.closingdate"
                persistent
                width="290px"
              >
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field
                    :disabled="enabled"
                    v-model="currentevent.closingdate"
                ...
                >
                  <v-spacer></v-spacer>
                  <v-btn text color="info" @click="cmodal = false"
                    >Cancel</v-btn
                  >
                  <v-btn
                    text
                    color="info"
                    @click="$refs.dialog3.save(currentevent.closingdate)"
                    >OK</v-btn
                  >
                </v-date-picker>
              </v-dialog>
        ...
        >
          <v-text-field
            v-model="currentevent.contactname"
            :error-messages="errors"
        ...
        <ValidationProvider
          v-slot="{ errors }"
          name="Phone"
          :rules="{ required: true, regex: /^[\d-\s]{9,}$/ }"
          autocomplete="off"
        >
          <vue-tel-input-vuetify
            v-model="currentevent.contactnum"
            :error-messages="errors"
        ...
        <ValidationProvider
          v-slot="{ errors }"
          name="Total Volunteers Needed"
          rules="required"
          autocomplete="off"
        >
          <v-text-field
            type="number"
            v-model="currentevent.totalvolunteer"
            :error-messages="errors"
        ...
        <v-card-actions>
          ...
          <v-btn text color="blue darken-3" @click="handleUpdate">
            Update
          </v-btn>
        </v-card-actions>
      </v-form>
  ...
  methods: {
    handleUpdate() {
      ...
      if (this.currentevent.datefrom != null)
        this.eventform.datefrom = this.getLocaleDate(
          this.currentevent.datefrom
        );
      ...
      if (this.currentevent.closingdate != null)
        this.eventform.closingdate = this.getLocaleDate(
          this.currentevent.closingdate
        );
