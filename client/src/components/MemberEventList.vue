    ...
    <v-container fluid>
        ...
        <v-card class="cardAlert">
          <v-card-actions>
            ...
            <v-btn
              ...
              dark
              color="cyan"
              @click="updateMessageDialog = false"
            >
      ...
      <v-dialog v-model="deleteEventDialog" max-width="300px">
        ...
        <v-card>
          <v-card-text>
            <v-row class="mx-0"
              >This investment record will be deleted. Are you sure?</v-row
            >
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
      ...
      <v-card>
        <v-card
          color="grey darken-2"
          elevation="6"
          class="v-card-header-white d-flex grow flex-wrap pa-6"
        >
          My Campaign(s)
          <v-spacer></v-spacer>
          <v-text-field
        ...
        >
          ...
          <template v-slot:[`item.action`]="{ item }">
            ...
            <v-icon
              color="cyan darken-3"
              small
              @click="openDeleteEventDialog(item.eid)"
            >
              mdi-delete
            </v-icon>
          </template>
          <template v-slot:expanded-item="{ headers, item }">
            <td :colspan="headers.length">
              ...
              <p>
                {{ getLocaleDate(item.datefrom, true) }}<strong> | </strong
                >{{ getLocaleDate(item.dateto, true) }}<strong> | </strong
                >{{ getLocaleDate(item.closingdate, true) }}
              </p>
              <br />
              <h3>Location</h3>
              <p>{{ item.location }}</p>
