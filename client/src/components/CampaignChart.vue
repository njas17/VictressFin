  ...
  <v-card class="mt-4 mx-auto mb-5" max-width="500">
    ...
    >
      <v-sparkline
        ...
        smooth
        :labels="Object.keys(countResult)"
        :value="Object.values(countResult)"
        color="white"
        line-width="2"
        padding="16"
      ></v-sparkline>
      <v-card-title small v-if="Object.keys(countResult).length < 2">
        ...
        <span class="title font-weight-light white--text"
          >{{ Object.values(countResult)[0] || 0 }} applicant(s)
          <span v-if="Object.keys(countResult).length === 1"> on </span>
          {{ Object.keys(countResult)[0] || null }}</span
        >
      </v-card-title>
    </v-sheet>
...
export default {
  ...
  computed: {
    countResult() {
      ...
      const filterVol = this.volunteers.filter(
        (e) => this.getLocaleDate(fromDate) < e.dateapp
      );

      let shortDate = "";
      const groupByDate = filterVol.reduce((prevAcc, curr) => {
        shortDate = new Date(curr.dateapp).toLocaleString("en-MY").slice(0, 2);
